# visualize.py
# Visualization of adjacency matrix and adjacency list.
# Supports: skipping saving if file already exists (overwrite=False)
# And in fallback mode (without networkx) saves adjacency list to .txt if a path is provided.
...
def visualize_adjacency_matrix(mat: List[List[Any]],
    ...
    """
    Visualization of adjacency matrix as a heatmap.
    - save_path: if provided, attempt to save; if file exists and overwrite==False — skip saving.
    """
...
def adjacency_list_to_matrix(adj: Dict[int, List[Any]], n: Optional[int] = None) -> List[List[int]]:
    """
    Convert adjacency list to 0/1 matrix (ignores weights).
    """
    max_v = -1
...
def visualize_adjacency_list(adj: Dict[int, List[Any]],
    ...
    - If as_matrix=True, first shows/saves the matrix (via visualize_adjacency_matrix).
    - If networkx is installed, draws the graph and saves to save_graph_path (if provided).
    - If networkx is not installed, if save_adj_txt_path is provided — saves adjacency list to .txt,
      otherwise prints to console.
    - overwrite: if False and file exists — skip saving of that file/image.
    Returns a dict with save flags: {"matrix": bool, "graph": bool, "adj_txt": bool}
    """
    ...
    if _HAS_NX:
        ...
        # networkx is not installed — save to txt if path provided, otherwise print
